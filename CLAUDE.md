...
- WebMIDI input is decoded in `src/wasm/js/player-midi.ts` and applied through the studio view's existing `setStep` / `setDeviceParam` paths — C++ knows nothing about MIDI. Shareable `?p=` studio patches (fflate-compressed edits layered on a `?play=` deep link) live in `src/lib/studio-patch.ts`; they carry edits only, never the song.
